...
War::is_over() const
{
  return m_player1.is_empty() || m_player2.is_empty(); // || m_turns >=10'000;
}
...
War::turn()
{
  ...
  if (rank(c1) > rank(c2))
  {
    m_pile.put_bottom(m_pile1);
    m_pile.put_bottom(m_pile2);
    claim(m_player1);
  }
...
War::war()
{
  ...
  if (!m_player1.is_empty())
    m_pile1.put_top(flip(m_player1));
  if (!m_player2.is_empty())
    m_pile2.put_top(flip(m_player2));
}
